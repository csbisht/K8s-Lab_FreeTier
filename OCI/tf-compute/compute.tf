resource "oci_core_instance" "ubuntu_instance_master" {
  ...
  provisioner "remote-exec" {
    inline = [
          ...
          "export getline=`grep -n \"containerd.runtimes.runc.options\" /etc/containerd/config.toml |cut -d':' -f1`",
          "export nextline=`expr $${getline} + 1`",
...
resource "oci_core_instance" "ubuntu_instance_node" {
  ...
  provisioner "remote-exec" {
    inline = [
          ...
          "echo 'deb [signed-by=/usr/share/keyrings/kubernetes-archive-keyring.gpg] https://apt.kubernetes.io/ kubernetes-xenial main' | sudo tee /etc/apt/sources.list.d/kubernetes.list",
          "sudo apt-get update",
          "sudo apt-get install -y kubelet=1.19.0-00",
